...
for rel in [
    ...
    if d:
        ...
        if d.get("version") != "1.0.1": errors.append(f"{rel}: version mismatch")

for rel in ["gemini-extension.json", "openclaw.plugin.json"]:
    d = load(rel)
    if d and d.get("version") != "1.0.1": errors.append(f"{rel}: version mismatch")
...
for p in ROOT.rglob("*.json"):
    ...
    if isinstance(d, dict) and "version" in d and d["version"] != "1.0.1":
